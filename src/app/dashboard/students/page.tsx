...
function StudentManagementContent({ isAdmin }: { isAdmin: boolean }) {
    ...
    const handleMigration = async () => {
        if (!confirm("Jalankan perbaikan data (Tingkat) untuk semua siswa?")) return;
        try {
            const count = await adminService.migrateTingkatData();
            setSummary(`Migrasi selesai. ${count} data diperbarui.`);
            refresh();
        } catch (e: any) {
    ...
    const startEdit = (student: Student) => {
        ...
        setFormData({
            ...
            tingkat: student.tingkat ? student.tingkat.toString() : "10",
            program: "",
            kompetensi: ""
        });
    };
